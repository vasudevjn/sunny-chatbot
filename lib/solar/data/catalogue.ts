/**
 * ============================================================================
 * WAAREE ENERGIES CATALOGUE — part real, part estimated. READ THIS HEADER.
 * ============================================================================
 * Sourced 2026-09-06 from Waaree's own published product pages, now indexed in
 * the knowledge base (`Waaree-2kW-OnGrid-Bifacial-DCR-Kit`,
 * `Waaree-Radiance-Lite-3kW-OnGrid-Kit`, `Waaree-Solar-Panel-Range`).
 *
 * VERIFIED against those pages:
 *   - Product and model names, including the BiN-08 series and the Non-DCR
 *     580 Wp N-type module
 *   - Two kit prices: 2 kW Bifacial DCR at Rs 91,999 (MRP Rs 126,000) and
 *     Radiance Lite 3 kW at Rs 140,999 (MRP Rs 152,799)
 *   - Module warranty (12 yr product / 30 yr power output) and the
 *     WS On Grid 1-Phase 3 kVA Gen4 inverter
 *   - DCR status: the 2 kW kit is sold as "Bifacial DCR"; Radiance Lite uses
 *     "BiN-08-560/565/570/575/580 (DCR)"; the 580 Wp N-type is sold as Non-DCR
 *
 * ESTIMATED, NOT VERIFIED — every one of these still needs a real price list:
 *   - `priceInr` for every package. Waaree's kit pages state "Structure &
 *     Installation Charges Not Included", so a kit price is NOT an installed
 *     system price. Each package below is `kit price + Rs 15,000/kW` for
 *     structure, BOS and installation. That allowance is a guess. Using the
 *     bare kit price would put the system near Rs 46,000/kW and reproduce the
 *     implausible ~2-year Maharashtra payback this project already hit once.
 *   - Prices for every package except pkg-2kw-value and pkg-3kw-standard,
 *     which are scaled from those two real figures.
 *   - `efficiencyPct` on all panels and inverters (not published on the pages).
 *   - The 5 kVA and 8 kVA inverters — inferred members of the WS Gen4 family.
 *     Only the 3 kVA is confirmed.
 *   - Panel counts where the page gave a kit size but not a module count.
 *
 * CONFLICT IN THE SOURCE: the kit pages state a 12 yr product / 30 yr power
 * output module warranty and an 8 yr inverter warranty in the header, but
 * "10 years on panels, 5 years on inverter" in the Radiance Lite spec table.
 * The conservative figures are used here. Confirm before quoting a warranty.
 *
 * The one field that must never be wrong is `dcrCompliant`. PM Surya Ghar
 * requires domestically manufactured modules, so recommending a non-DCR package
 * to a homeowner claiming the subsidy costs them up to Rs 78,000. The matcher
 * hard-filters on it; get it right per SKU.
 * ============================================================================
 */
