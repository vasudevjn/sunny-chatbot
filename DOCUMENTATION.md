...
|---|---|---|---|
| **Suggested prompts strip** | Chips above the chat offering the next useful question, generated from what was just discussed (e.g. the homeowner's own state, system size or tier), with a static table as fallback. "What would I save?" at the start. "Explain the subsidy" once sized. "Get my draft proposal" once products are shown. | **Attract → Engage** Removes the blank page problem and steers a curious visitor into the qualification funnel instead of letting them bounce. | app/parts/suggested-prompts.tsx, lib/ai/suggestions.ts, SUGGESTED_PROMPTS in config.ts, stage from lib/solar/lead-store.ts |
...
**Suggested prompts:** A homeowner who does not know what solar costs also does not know what to ask an AI. The suggested prompts chips solve this problem and creates a more engaging customer journey. The suggestions reflect the actual conversation: what was just discussed and the homeowner's specific numbers while keeping the static table as a fallback so the strip can never break or go empty. The chips are generated per turn, constrained to a small zod schema so output stays bounded rather than free-form and told explicitly which tools/features are disabled so it cannot suggest a question Sunny has no way to answer. 
...
Sample electricity bill photos for testing this feature are in [samples/](samples).
